...
def draw_heatmap(img, img_id, id_obj, img_depth, point_depth, center_coordinate, heatmap_template, side, width, height):
    ...
    id_obj_map = np.ones_like(need_map)
    depth_obj_map = np.ones_like(need_map)

    if type(id_obj) is not int:
        id_obj_map *= np.array(id_obj.cpu(), dtype=id_obj_map.dtype)
    else:
        id_obj_map *= id_obj
    id_obj_map *= (need_map != 0)

    if type(point_depth) is not int:
        depth_obj_map *= np.array(point_depth.cpu(), dtype=depth_obj_map.dtype)
    else:
        depth_obj_map *= point_depth
    depth_obj_map *= (need_map != 0)

    # if type(id_obj) is not int:
    #     img_id[y1:y2,x1:x2] = np.array(id_obj.cpu())
    # else:
    #     img_id[y1:y2,x1:x2] = id_obj
    img_id[y1:y2,x1:x2] = id_obj_map
    img_depth[y1:y2,x1:x2] = depth_obj_map

    return img, img_id, img_depth
...
def generate_gassian_heatmap(pred_tracks, pred_visibility=None, image_size=None, side=20):
    ...
    num_frames = len(pred_tracks)   # pred_tracks: (b, f, num_points, 2); num_points vary in different frames
    ...
    for frame_idx in range(num_frames):
        ...
        img_id = np.zeros((height, width), np.float32)
        img_depth = np.zeros((height, width), np.float32)
        for point in pred_tracks[frame_idx]:
            py, px, id_obj, point_depth = point
            ...
            img, img_id, img_depth = draw_heatmap(img, img_id, id_obj, img_depth, point_depth, (px, py), heatmap_template, side, width, height)
        ...
        img_id = torch.from_numpy(img_id).permute(2, 0, 1).contiguous()
        img_depth = torch.from_numpy(img_depth).permute(2, 0, 1).contiguous()
        img = torch.cat([img, img_id, img_depth], dim=0)
...
def sample_trajectories(
    ...
    if last_visible:
        ...
        last_visible_mask = pred_visibility[:, -1] * (pred_tracks[:, -1, :, 0] > 0) * (pred_tracks[:, -1, :, 1] > 0)
        mask = mask * last_visible_mask
