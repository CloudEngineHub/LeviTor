...
class ControlNetConditioningEmbeddingSVD(nn.Module):
    ...
    def __init__(
        ...
        if self.with_id_feature:
            self.mlp_embedding = SparsePointAdapter(
                embedding_channels=1280,
                channels=[feature_out_channels[0], ],
                downsample_rate=[8,],
                mid_dim=128,
            )

            self.conv_in_feat = nn.Conv2d(feature_channels, feature_out_channels[0], kernel_size=3, padding=1)
            ...
            for i in range(len(feature_out_channels) - 1):
                ...
                self.blocks_feat.append(nn.Conv2d(channel_in, channel_in, kernel_size=3, padding=1))
                # self.blocks_feat.append(nn.Conv2d(channel_in, channel_out, kernel_size=3, padding=1, stride=2))
                self.blocks_feat.append(nn.Conv2d(channel_in, channel_out, kernel_size=3, padding=1, stride=1))

            self.conv_out_feat = zero_module(
                nn.Conv2d(feature_out_channels[-1], conditioning_embedding_channels, kernel_size=3, padding=1)
            )
    ...
    def forward(self, conditioning, point_embedding=None, point_tracks=None):
        ...
        if self.with_id_feature:
            assert point_embedding is not None
            id_feature = self.create_id_feature_from_point_embedding(point_embedding, point_tracks, size=(width, height))

            # batch_size, frames, channels, height, width = id_feature.size()
            # id_feature = id_feature.view(batch_size * frames, channels, height, width)

            embedding_feat = self.conv_in_feat(id_feature)
            embedding_feat = F.silu(embedding_feat)

            for block in self.blocks_feat:
                embedding_feat = block(embedding_feat)
                embedding_feat = F.silu(embedding_feat)

            embedding_feat = self.conv_out_feat(embedding_feat)
            
            # # split them apart again
            # # actually not needed
            # new_channels, new_height, new_width = embedding_feat.shape[1], embedding_feat.shape[2], embedding_feat.shape[3]
            # embedding_feat = embedding_feat.view(batch_size, frames, new_channels, new_height, new_width)

            if embedding.shape[0] // embedding_feat.shape[0] == 2:
                embedding_feat = torch.cat([embedding_feat] * 2)

            embedding = embedding + embedding_feat

        return embedding

    def create_id_feature_from_point_embedding(self, point_embedding, point_tracks, size):
        out = self.mlp_embedding(point_tracks, size, point_embedding=point_embedding, drop_rate=0.0, loss_type='global')

        if self.mlp_embedding.training:
            adapter_state, loss_mask = out
        else:
            adapter_state = out

        assert len(adapter_state) == 1
        # (f, c, h, w)
        id_feature = adapter_state[0]
